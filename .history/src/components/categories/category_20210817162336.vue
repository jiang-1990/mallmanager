<template>
  <el-card class="box-card">
    ...
    <!--添加分类-->
    <el-button class="addClass" type="primary" plain @click="showAddDia">添加分类</el-button>
    <!--分类表格-->
    <el-table :data="tableData" style="width: 100%" height="430px">
      <el-tree-grid
        label="分类名称" 
        prop="cat_name"
        treeKey="cat_id"
...
export default {
  ...
  methods: {
    ...
    async addClass(){
      ...
      this.addForm.cat_pid=this.selectedOptions[this.selectedOptions.length]
      if(this.selectedOptions.length===0){
        this.addForm.cat_pid=0
      }else if(this.selectedOptions.length===1){
        this.addForm.cat_pid=2
      }else{
        this.addForm.cat_pid=3
      }
